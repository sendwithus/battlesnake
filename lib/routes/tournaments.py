from lib.server import json_response, app
...
def tournaments_create():
    ...
    return json_response({}, msg='Tournament created', status=201)


@app.route('/api/tournaments/<tournament_id>')
def tournament_details(tournament_id):
    """ Get data for a single tournament """
    return json_response(msg='Tournament not found', status=404)
...
def tournament_game_create(tournament_id):
    ...
    return json_response({}, msg='Game created', status=201)
...
def tournament_game_detail(tournament_id, game_id):
    ...
    return json_response(msg='Game not found', status=404)
